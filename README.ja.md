...
あなたのコーディングエージェント——**Codex**、**Claude Code**、**OpenClaw**、
**OpenCode**、**Hermes**、**DeepSeek Harness**——がターンを終えると、デスクトップの
小さな石積み（3つの川石）のそばにノートがそっと現れます。
...
- **跡をたどる。** ノートを開けば、そのターンが走っていた場所へ戻ります——正確な
  ...
  DeepSeek Harness のノートは、特定の会話deep linkではなく、Webホームへベストエフォートで戻ります。
...
Cairnは何も計測しません。アカウントも、解析も、テレメトリもありません。
ネットワーク通信は1日1回、GitHub Releasesへの確認だけで、ノートの内容は送りません。

それがこの製品の芯であり、同時に、あなたが書いてくれない限り、これが役に立って
いるのかを知る術がないということでもあります。
**[Cairnは実際に役に立っていますか](https://github.com/quentinzhang/cairn/discussions/1)**

質問は三つ。日本語・English・中文のいずれでも構いません。登録は不要で、
望まないかぎり追って連絡することもありません。
