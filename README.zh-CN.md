...
Cairn 不与代理集成——**它只是读一个目录**。任何能写 JSON 文件的东西都能留下便签，所以支持一个新的运行时不需要改动这个应用。见 [inbox 协议](docs/inbox-protocol.md)。
...
桥接器被设计成静默失败——完成 hook 绝不能破坏它所在的代理——所以有一个工具，它的全部职责就是解释这份沉默：

```bash
python3 /Applications/Cairn.app/Contents/Resources/cairn_doctor.py
```

对你实际安装的每个运行时，它都会指名原因和修法：hook 指向了已经移动的位置、插件已链接但未启用、inbox 里卡着畸形负载、第二个应用副本在偷便签。加 `--probe` 可以端到端追踪一条测试便签。它的输出可以安全地贴进 issue——没有便签正文，没有提示词，没有应用之外的路径。
...
CI 流水线、一次长时间构建、另一个代理运行时——对照 [`docs/inbox-protocol.md`](docs/inbox-protocol.md) 写一个生产者即可。最短的版本只有一行：
...
<details>
<summary><b>在终端里</b>——同样的设置，以及 <code>cairn-save</code> 技能</summary>

连接窗口做的每一件事都出自同一个脚本，它就打包在应用内部：
...
./Scripts/build_app.sh && open dist/Cairn.app # 打包并运行
python3 Scripts/cairn_reset.py                # 查看"恢复初次运行"会清掉什么
```

构建需要 Xcode 16+。没有任何依赖要拉取——只用系统框架和 Python 3 / Node.js 标准库。

首次运行的流程只会发生一次，因此最难测试。`cairn_reset.py` 把它整个走回去——断开所有代理，清空队列、偏好设置与隐私授权，但保留应用本身——下次启动就又是第一次启动。它默认只打印计划、不做任何改动，加 `--yes` 才执行；`--keep-permissions` 可保留授权。
