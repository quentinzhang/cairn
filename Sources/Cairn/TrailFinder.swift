    ...
    private static func followTrail(_ completion: CodexCompletion) async {
        ...
        if let app = hostApp(locator) {
            ...
            if let editorStatusExecutable = editorStatusExecutable(for: app) {
                ...
                // A known multi-window editor without an exact mapping must
                // not steal focus with whichever window it used last. This
                // happens when a session's extension host has exited, when
                // the editor changed its status format, or when two windows
                // have an identical title. Finder is less magical, but it is
                // honest and keeps the click from landing in the wrong task.
